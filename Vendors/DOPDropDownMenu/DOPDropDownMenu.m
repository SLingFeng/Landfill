...
- (UIColor *)indicatorColor {
    if (!_indicatorColor) {
        _indicatorColor = [CommonTools getFontNormalColor];
    }
...
- (UIColor *)textColor {
    if (!_textColor) {
        _textColor = [CommonTools getFontNormalColor];
    }
    return _textColor;
}
...
- (UIColor *)separatorColor {
    if (!_separatorColor) {
        _separatorColor = [CommonTools getFontNormalColor];
    }
    ...
    for (int i = 0; i < _numOfMenu; i++) {
        if (i != tapIndex) {
            [self animateIndicator:_indicators[i] Forward:NO complete:^{
                [weakSelf animateTitle:_titles[i] show:NO complete:^{
...
- (void)animateTableView:(UITableView *)tableView show:(BOOL)show complete:(void(^)())complete {
    if (show) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            _tableView.frame = CGRectMake(self.origin.x, self.frame.origin.y + self.frame.size.height, self.frame.size.width, tableViewHeight+20);
        }];
    } else {
        [UIView animateWithDuration:0.2 animations:^{

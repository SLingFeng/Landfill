...
///在友盟官网生成的AppKey传入
NSString *const AppKey = @"57b11bf0e0f55acc8c002f6d";
...
//    设置分享参数
    [UMSocialUIManager showShareMenuViewInView:nil sharePlatformSelectionBlock:^(UMSocialShareSelectionView *shareSelectionView, NSIndexPath *indexPath, UMSocialPlatformType platformType) {
        ...
        [[UMSocialManager defaultManager] shareToPlatform:platformType messageObject:messageObject currentViewController:self completion:^(id data, NSError *error) {
            ...
            if (!error) {
                [MyMBProgressHUD hudForText:@"分享成功" delay:2 view:weakView];
            }
            else{
                if (error) {
                    if ((int)error.code == 2010) {
//                        message = @"分享取消";
                        [MyMBProgressHUD hudForText:@"分享取消" delay:2 view:weakView];
                    }else {
...
//                    message = [NSString stringWithFormat:@"分享失败"];
                    [MyMBProgressHUD hudForText:@"分享失败" delay:2 view:weakView];
                }

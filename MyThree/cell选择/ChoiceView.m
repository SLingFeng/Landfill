...
//    }
    ...
    for (int x=0; x<needNum; x++) {
        ...
        MyButton *btn = [[MyButton alloc] initWithFontSize:26 fontColor:kAAAAAA fontText:@""];
//        btn.titleLabel.numberOfLines = 0;
        [self addSubview:btn];
        btn.hidden = 1;
        kViewBorderRadius(btn, 2.5, 0.5, kAAAAAA);
        [btn setTitleColor:kFFFFFF forState:(UIControlStateSelected)];
        [btn setBackgroundImage:[UIImage createImageWithColor:kFFFFFF] forState:(UIControlStateNormal)];
        [btn setBackgroundImage:[UIImage createImageWithColor:kBED500] forState:(UIControlStateSelected)];
...
//            [btn setImage:[UIImage imageNamed:@"radio_selected"] forState:(UIControlStateSelected)];
//        }
        GX_AddConfigInfoModel *obj = arr[i];
        NSString *title = obj.c_name;
        ...
        CGFloat btnW = [SLFCommonTools textSize:title font:[SLFCommonTools pxFont:26]].width + 28;
        CGFloat btnH = kAW(40);//[SLFCommonTools textHight:title font:[SLFCommonTools pxFont:26].pointSize width:btnW];
        if (lastView == nil) {
...
- (NSString *)getSelectID {
    ...
    for (MyButton * btn in self.btnArr) {
        if (btn.isSelected) {
            GX_AddConfigInfoModel *model = _dataArr[btn.tag - 10];
            [ids appendString:[NSString stringWithFormat:@"%@%@", [ids hasSuffix:@","]?@"":ids.length==0?@"":@",", model.value_id]];
        }
    }

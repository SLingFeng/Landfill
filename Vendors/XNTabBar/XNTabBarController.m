...
-(void)setBarBtn {
    ...
    for (int i=0; i<self.viewControllers.count; i++) { //根据有多少个子视图控制器来进行添加按钮
        ...
        UIImage *image = [UIImage imageNamed:_imageName[i]];
        UIImage *imageSel = [UIImage imageNamed:_imageNameSel[i]];
        
        [_myView addButtonWithImage:image selectedImage:imageSel title:_titles[i]];
    }
}
...
- (void)tabBar:(XNTabBar *)tabBar selectedFrom:(NSInteger)from to:(NSInteger)to {
    if (to == 3) {
        NSLog(@"sdhajskhfjdkh");
        UIAlertView * alert = [[UIAlertView alloc] initWithTitle:@"123" message:@"456" delegate:nil cancelButtonTitle:@"as" otherButtonTitles:@"qww", nil];
        [alert show];
        return;
    }
